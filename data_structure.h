	...
	public:
		...
		void size()
		{
			if(underflow())
				...
				cout<<"Size of Queue is: "<<r-f+1<<endl;
		}
};
template <class T>
...
class Heap
{
	...
	public:
		...
		void insert_Max(T val);
		void insert_Min(T val);
		T del_Max();
		T del_Min();
		void max_Heapify();
		void min_Heapify();
...
/*template <class T>
void DynamicArray<T>::computeSufixSum(DynamicArray<T> &sufixSumArray) const 
{
	sufixSumArray.clear();
	sufixSumArray.initialize_With_SizeAndValue(size,0);
	// Initialize the sufixSumArray with the last element of the input array
	sufixSumArray.array[size-1]=array[size-1];
	
	// Calculate the cumulative sum for the remaining elements
	for (size_t i = size-2; i >=0; i--) 
	{
	    sufixSumArray.array[i] = sufixSumArray.getElementAt(i+1) + array[i];
	}
